...
import { getWorkCollection, sortMDByDate } from '@/lib/utils/server.ts'
...
const imagesGlob = import.meta.glob<{ default: ImageMetadata }>(
  '/src/content/work/**/*.{jpeg,jpg,png,gif}' // add more image formats if needed
)

const renderContent = async (post: CollectionEntry<'work'>, site: URL) => {
    ...
    return async function (tree: Root) {
      ...
      visit(tree, 'image', (node) => {
        if (node.url.startsWith('/images')) {
          ...
          const imagePathPrefix = `/src/content/work/${post.id}/${node.url.replace('./', '')}`
...
const GET = async (context: AstroGlobal) => {
  const allPostsByDate = sortMDByDate(await getWorkCollection()) as CollectionEntry<'work'>[]
